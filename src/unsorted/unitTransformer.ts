...
export default function (program: ts.Program, pluginOptions: any) {
  ...
  if (!rootDir) throw "ERROR: UnitTransformer: No root dir found";
  if (!outDir) throw "ERROR: UnitTransformer: No out dir found";
  if (!extensions) throw "ERROR: UnitTransformer: no extensions in config";
  if (!Array.isArray(extensions))
    throw "ERROR: UnitTransformer: extension is expected to be an array";
